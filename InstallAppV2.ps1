...
$MSIArgumentsgit =@(
"/SP- /VERYSILENT /SUPPRESSMSGBOXES /FORCECLOSEAPPLICATIONS"
)

###  Blob storage url with sas key where all application stores for oragnisation 

$AppstoreUrl = "https://azappstore.blob.core.windows.net/installer?sp=rl&st=2023-02-23T07:28:11Z&se=2023-03-13T15:28:11Z&spr=https&sv=2021-06-08&sr=c&sig=%2BYs%2FVftSdIuvlvNVznN1MzFS2kvmWHDgKvpOu2BFPgE%3D"

# Configuring activate / desactivate Applications  install
$Isinstall_node = $FALSE
$Isinstall_git = $TRUE
$Isinstall_ajvcli = $FALSE
$Isinstall_markdownlint = $FALSE
$Isinstall_azurecli = $FALSE
$Isinstall_terraform = $FALSE
$Isinstall_python = $FALSE
$Isinstall_dotnetsdk = $FALSE
$Isinstall_openjdk8 = $FALSE
$Isinstall_openjdk11 = $FALSE
$Isinstall_mysqlserver = $FALSE
...
if ($Isinstall_git) {
    ...
    if ($git_current_version) {
        ...
        Try{
            start-Process $git_exe -ArgumentList $MSIArgumentsgit -Wait -PassThru 
